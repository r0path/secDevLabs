...
def brute_force(atk, user):
    if atk == "1":
        print(f"{Colors.YELLOW}The enumeration workflow is disabled in this build.{Colors.NORMAL}")
        return

    if atk == "2":
        print(f"{Colors.YELLOW}The password recovery brute-force workflow is disabled in this build.{Colors.NORMAL}")
        return
